...
for row in range(row_count):
    for column in range(column_count):
        ...
        button.grid(row=row+1, column=column) #row+1 because the first row is for the label
frame.pack()
    ...
    if value in right_symbols:
        ...
        if value == ".":
            ...
            if label["text"] == "0":
                ...
                label["text"] += value #append the digit to the end of the current number
...
window_x = int((screen_width/2) - (window.width/2))
window_y = int((screen_height/2) - (window.height/2))

#format 

window.geometry(f"{window.width}x{window.height}+{window_x}+{window_y}")
